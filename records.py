...
#class Header:
#    def __init__(self):
#        self.signature = bytes('TES4')
#        self.header_size = 0 #finalize
#        self.flag1 = struct.pack('<L', 0) #?
#        self.flag2 = struct.pack('<L', 0) #?
#        self.flag3 = struct.pack('<H', 0) #?
#        self.form_id = struct.pack('<L', 0) #?
#
#        self.hedr = bytes('HEDR')
#        self.hedr_size = struct.pack('<H', 0xC) #always 0xC bytes
#        self.version = struct.pack('<L', 0x3F800000)
#        self.record_count = 0 #finalize
#        self.next_object_id = struct.pack('<L', 0x00000800) #?
#
#        self.cnam = bytes('CNAM')
#        self.cnam_size = 0 #finalize
#        self.author = bytes('')
#
#        self.master = bytes('MAST')
#        self.master_size = 0 #finalize
#        self.master_file = bytes('')
#
#        self.data = bytes('DATA') #? not included in self.header_size
#        self.data_size = struct.pack('<H', 8) #not included in self.header_size
#        self.data1 = struct.pack('<L', 0) #? not included in self.header_size
#        self.data2 = struct.pack('<L', 0) #? not included in self.header_size
#
#        self.groups = []
#
...
#        self.author = bytes(author) + '\x00'
#        self.cnam_size = struct.pack('<H', len(self.author))
#
#    def set_master(self, file):
#        self.master_file = bytes(file) + '\x00'
#        self.master_size = struct.pack('<H', len(self.master_file))
#
#    def finalize(self):
#        self.record_count = struct.pack('<L', self.record_count)
#
#        list_to_calc = [
#            self.flag1,
#            self.flag2,
...
#            self.header_size += len(item)
#
#        self.header_size = struct.pack('<H', self.header_size)
#
#        self.packed = bytes('')
#        self.packed += (
#            self.signature +
